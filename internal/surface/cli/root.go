...
func newCheckoutUpdateClientProfileCommand(bootstrap Bootstrapper, options *rootOptions) *cobra.Command {
	var brand string
	var orderFormID string
	...
	command := &cobra.Command{
		Use:   "update-client-profile",
		Short: "Attach client profile data to a VTEX Checkout orderForm",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			profile, err := decodeRunInput([]byte(inputJSON))
			if err != nil {
				return err
			}
			...
			pipeline := execution.NewPipeline(application.Registry)
			envelope, err := pipeline.Execute(cmd.Context(), execution.ExecuteRequest{
				CapabilityID: checkout.CapabilityUpdateClientProfileID,
				Input: capability.Input{
					"brand":         brand,
					"orderFormId":   orderFormID,
					"clientProfile": map[string]any(profile),
				},
				Profile:       application.Config.Profile,
				CorrelationID: options.correlationID,
				Confirmed:     confirmed,
